...
def train(args):
    ...
    optimizer = optim.Adam(list(main_nets[0].parameters()) + list(main_nets[1].parameters()))
    ...
    for iter_count in range(args.run):
        ...
        if batch_size < len(replay_memory):
            ...
            error0 = (batch_target0.detach() - batch_estimate0).pow(2)
            ...
            error1 = (batch_target1.detach() - batch_estimate1).pow(2)

            loss = torch.mean(error0) + torch.mean(error1)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
