...
def train(args):
    ...
    for iter_count in range(args.run):
        ...
        if batch_size < len(replay_memory):
            ...
            target = torch.zeros(batch_size, args.length, args.width)
            estimate = torch.zeros(batch_size, args.length, args.width)

            for index in range(args.length * args.width):
                idx = int(index / args.width)
                idy = index % args.width
                with torch.no_grad():
                    target[:, idx, idy] = b_costs[:, idx, idy] + args.gamma * torch.min(target_nets[index](b_next_states[:, idx, idy, :]), dim=-1)[0]
                estimate[:, idx, idy] = main_nets[index](b_curr_states[:, idx, idy, :])[range(batch_size), b_curr_phases[:, idx, idy]]
