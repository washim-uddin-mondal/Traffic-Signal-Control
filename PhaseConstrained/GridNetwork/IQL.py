...
def train(args):
    ...
    for iter_count in range(args.run):
        ...
        cost = torch.sum(next_state[:, :4], dim=-1)
        ...
        replay_memory.append([curr_state.clone(), next_state.clone(), cost/args.scaling])
        ...
        if batch_size < len(replay_memory):
            ...
            target = torch.zeros(batch_size, IntersectionN)
            estimate = torch.zeros(batch_size, IntersectionN)
            ...
            for index in range(IntersectionN):
                with torch.no_grad():
                    ...
                    target[:, index] = b_costs[:, index] + args.gamma * (t_out0 + t_out1 + t_out2)
                estimate[:, index] = main_nets[index](b_curr_states[:, index, :])[range(batch_size), b_curr_actions[:, index].long()]
