"""Deep Q Learning for Grid intersections. All intersections learn via VDN.
"""
import os
import torch
        ...
        if batch_size < len(replay_memory):
            ...
            for index in range(IntersectionN):
                with torch.no_grad():
                    ...
                    target += b_costs[:, index] + args.gamma * (t_out0 + t_out1 + t_out2)
                estimate += main_nets[index](b_curr_states[:, index, :])[range(batch_size), b_curr_actions[:, index].long()]
